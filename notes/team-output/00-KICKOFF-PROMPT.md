...
- **Prompt-context independence for verification.** AI can write any leg of the triad — property tests, oracle examples, adversarial inputs — *but* the verification artifacts must come from a separate session than the one that wrote the implementation. Same model is fine; shared chat history is not. The implementation session reasons from "what I've already coded"; the verification session reasons from the SPEC. Aaron's role on verifier-correctness paths (`compiler/src/main/kotlin/.../verifier/`, `…/symboltables/SymbolTable.kt`) is PR review, not authorship — push back if a property invariant or oracle example reads wrong.
