...
| P11 | Hand-checked type-inference cases + existing examples (no semantic change should land in existing goldens) | 20+ cases like `y := add(1, 2)` infers `y: int`. Cases drafted in a fresh-session prompt against the SPEC, then reviewed by Aaron before being canonized as the oracle. Reject any program where inference disagrees with the oracle. (Type-(b).) |
...
| P14 | Full publishing pipeline end-to-end: install in fresh containers, run smoke tests, compare across three artifacts | Use scratch Docker containers. Smoke tests cover idiomatic-output expectations (source maps resolve, `.pyi` is well-formed, C header compiles in a hello-world). |
...
### The discipline rule: prompt-context independence

The triad is the discipline. AI can write any of the three legs, *including* property-test invariants and differential-oracle examples. The non-negotiable rule is **prompt-context independence**: the agent that writes a verification artifact MUST run in a separate session from the agent that wrote the corresponding implementation, with no shared chat history. Same model, different conversation.

- **Implementation session**: has the SPEC + the current code in context. Writes the impl.
- **Verification session**: has the SPEC + a *blank* code context (no shared chat). Writes property tests, differential-oracle examples, or adversarial inputs against the SPEC, not against the impl.

This preserves the "tests pass, code is wrong" defense: if both sessions reason from the same chat context, they share the same silent assumptions and the verification overfit becomes invisible. Separate sessions = independent reasoning = the failure mode surfaces.

Aaron's role is the *reviewer*, not the *author*: read the verification artifacts as PR review, especially when they're concerning verifier-correctness paths (`compiler/src/main/kotlin/.../verifier/`, `…/symboltables/SymbolTable.kt`). If a property invariant or oracle example reads wrong, push back; that's where R8 hides.

Weekly time budget: **~4 h/week** of focused review on verification PRs + ~2 h/week running fresh-session adversarial reviews. If sustainable < 4 h/week, the timeline is wrong; surface to §6 escalation.
...
**Did any leg of the verification triad surface an issue this week?** Track in `IMPLEMENTATION-LOG.md`. A healthy week sees at least one of: a property-test failure (caught a real bug, not flaky), a differential-oracle disagreement, or an adversarial-input "compiler-broke" case. **If "no" across all three legs for 2 weeks running: the triad is theater. Stop feature work; do a full triad design review for the current phase — likely the property invariants are too loose, the oracle isn't strict enough, or the adversarial prompt is too tame.** This is the dominant tell that the AI-augmented compression has gone fake.
...
| "Verification triad surfaced any issue this week" | Leading | Yes, every week | No across all three legs, 2 weeks running | R8 stop-the-line (triad is theater) |
