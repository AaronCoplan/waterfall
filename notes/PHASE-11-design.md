...
- `compiler/src/main/kotlin/com/aaroncoplan/waterfall/compiler/verifier/StatementVerifier.kt` — `verifyVarAssignment` line 97 + `verifyIncrement` line 112 emit `UnknownIdentifier(name, Context.{ASSIGNMENT_LHS|INCREMENT_TARGET}, s.getSourcePosition())` on null lookup. Drop the silent no-op `return emptyList()`.
- `compiler/src/main/kotlin/com/aaroncoplan/waterfall/compiler/verifier/Elaboration.kt` — for `Kind.IDENTIFIER`, no longer write `VoidType` for unresolved names. The new contract per §2.2: if the verifier rejected the identifier earlier, Elaboration never sees it; if Elaboration sees an unresolved name anyway, it throws ISE (drift bug). Implementation note: change the `?: WaterfallType.VoidType` at `Elaboration.kt:151` to `?: error("Elaboration: unresolved identifier '$name' after verifier should have rejected; verifier+elaboration drift at ${fallbackPos.generateMessage()}")`. (`expr.sourcePosition` is the §4.2 form — per-expression positions don't exist yet in §4.1; use `fallbackPos` from the enclosing `lowerStatement` call.)
...
**Files changed (Main.kt / IrLowering — defensive assertions):**
- `compiler/src/main/kotlin/com/aaroncoplan/waterfall/compiler/ir/IrLowering.kt` — for `Kind.IDENTIFIER`, replace the `?: WaterfallType.VoidType` fallback at line 191 with: `?: error("IrLowering: identifier '$name' missing from resolvedTypes at ${fallbackPos.generateMessage()}; verifier or Elaboration drift")`. Additionally, add a post-fetch assertion: `if (waterfallType is WaterfallType.VoidType) error("IrLowering: identifier '$name' resolved to Void at ${fallbackPos.generateMessage()}; ModuleVerifier should have emitted UnknownIdentifier")`. **The two assertions are different: the first catches missing entries; the second catches Void-poisoned entries.** (`fallbackPos` is the statement-level position available in §4.1; `expr.sourcePosition` replaces it in §4.2 once per-expression positions land.)

**Files added (tests):**
- `compiler/src/test/kotlin/com/aaroncoplan/waterfall/compiler/verifier/UnknownIdentifierTest.kt` — ≥10 cases covering each `Context` variant:
...
- `compiler/src/main/kotlin/com/aaroncoplan/waterfall/compiler/verifier/Elaboration.kt` — update `LAMBDA_POS` references where applicable to use `expr.sourcePosition` instead. **OQ-11.7 escalation point**: see §6.
