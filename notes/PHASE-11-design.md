...
- **Per OQ-11.3=(a) ACCEPTED** (spec line ~915): Elaboration takes `MutableList<VerifyError>` and emits `UnknownIdentifier` directly into it for expression-context unresolved names (IDENTIFIER / ARRAY_INDEX / FUNCTION_CALL.LOCAL). VoidType is still written to the resolvedTypes table to keep the table invariant. Elaboration does **NOT** throw ISE — that was the pre-OQ-11.3 "rejected Option B" framing. The ISE language in earlier drafts of this section referred to a design that was superseded by the OQ-11.3=(a) decision.
- `IrLowering.kt:187-192` (the M7 fallback that reads VoidType) has been replaced with two IDENTIFIER-specific drift assertions: (1) missing-entry assertion (`?: error("...verifier or Elaboration drift")`); (2) Void-entry assertion (`if (waterfallType is VoidType) error("...ModuleVerifier should have emitted UnknownIdentifier")`). Both assertions are **defensive drift catches only** — they never fire under normal operation because `Main.kt` short-circuits at `!verifyResult.isSuccessful` before `IrLowering` is invoked. **OQ-11.2** (resolved-in-spec): the assertions fire only for IDENTIFIER nodes — VoidType continues to be valid for `NullLiteral`, lambda return type, etc.
